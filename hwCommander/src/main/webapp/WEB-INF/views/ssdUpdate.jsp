...
<html>
<head>
<title>HWCommander - SSD Update</title>
<!-- Required meta tags -->
<meta charset="utf-8">
<!-- Bootstrap CSS -->
...
<script>
    $(function() {
    	dataSetting();
        $('#btn_ssd_update').on("click", function () {
        	if(!validationCheck()) {
        		return false;
        	}
...
function dataSetting() {
	...
	$("#basic").val("${selectData.basic}");
	$("#fnc").val("${selectData.fnc}");
	$("#cmf").val("${selectData.cmf}");
	$("#war").val("${selectData.war}");
	$("#thr").val("${selectData.thr}");
	$("#rlb").val("${selectData.rlb}");
...
function goSsdUpdate() {
    var form = $("#ssd_update_form").serialize();
    
    $.ajax({
        type: "post",
        url: "/admin/ssdUpdateLogic.do",
        data: form,
        dataType: 'json',
                        ...
                        <div class="card mb-4">
							<div class="card-body">
                               <form id="ssd_update_form">
                                   ...
                                   <div class="mt-4 mb-0">
                                       <div class="d-grid"><a class="btn btn-secondary btn-block" id="btn_ssd_update">Update</a></div>
                                   </div>
                               </form>
                           </div>
                        </div>
