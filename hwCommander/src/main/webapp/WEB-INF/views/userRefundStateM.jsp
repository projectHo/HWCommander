...
<head>
<title>HWCommander - 환불 요청 현황</title>
<!-- Required meta tags -->
<meta charset="utf-8">
<!-- Bootstrap CSS -->
			...
			<div class="w-100 estimateCalc_background pt-5 pb-5 ps-2 pe-2" style="width: 99%;">
				...
				<c:forEach items="${refundInfoVOList}" var="item">
					<c:forEach items="${orderMasterVOList}" var="orderItem">
						<c:if test="${item.orderId == orderItem.id}">
							<div class="cards mb-2 p-2">
								<h3 class="pt-1 mb-0">${orderItem.orderDateStr}</h3>
								<div class="goods-box p-3 pb-2">
									<div class="goods-header">
										<div class="goods-name">상품명 : ${orderItem.orderName}</div>
									</div>
									<div class="goods-body pb-2">
										<div class="goods-state">배송현황 : <small>${orderItem.orderStateCdNm}</small></div>
										<div class="goods-num">주문번호 :${orderItem.id}</div>
