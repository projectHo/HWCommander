...
<script>
	...
	function clickAnswerBtn(el){
		if($(el).children().html() === "좋아요!"){
			sessionStorage.setItem("data-8",0);
		}else if($(el).children().html() === "싫어요!"){
			sessionStorage.setItem("data-8",1);
		}else {
			sessionStorage.setItem("data-8",2);
		}
	}
	function clickReturnBtn(){
		sessionStorage.setItem("data-8","");
	...
	// 견적산출 데이터처리부(수신)
	if(sessionStorage.getItem("data-8")){
		if(sessionStorage.getItem("data-8") === "0"){
			$("#answer-a").prop("checked",true);
		}else if (sessionStorage.getItem("data-8") === "1"){
			$("#answer-b").prop("checked",true);
		}else if (sessionStorage.getItem("data-8") === "2"){
